...
with high SALI values. To test this out, we split the Cavalli dataset into two groups - low and high
...
better at the higher end of the SALI values. However in all three cases, systematic prediction
errors are apparent. Based on these observations, we arbitrarily chose $f_{\textrm{diff}}$ as the
aggregation function for subsequent analyses. Table \ref{tab:rfmodel-orig} and Figure
\ref{fig:rfmodel-orig} summarizes the performance of the models built using the reported activity
data (original, single molecule activity) for the Costanzo, Kalla and Dai datasets. Each of these
models employed the entire dataset (rather than splitting into a training and testing set) and
...
focuses on \emph{pairwise} datasets -- which are usually large enough for a statistically
significant model.
...
\ctable[cap={}, caption={A comparison of the pairwise descriptor distribution for low and high SALI
  ...
  $f_{diff}$ aggregation function and Euclidean distances were evaluataed using all the descriptors
  in the pool. The \emph{low} group is defined as those observations with SALI
  less than 2.03 and the remainder is assigned to the \emph{high} group.},
